...
class Trinomial_Pricing(Option_Pricing):
    ...
    def generate(self):
        ...
        for i in range(1, self.steps + 1):
            for j in range(2 * i + 1):
                if j == 0:
                    ...
                    self.ST[self.steps - i + j, i] = self.ST[self.steps - i + j - 1, i - 1] * m
        ...
        for i in range(self.steps - 1, -1, -1):
            for j in range(2 * i + 1):
                self.call_option_values[self.steps - i + j, i] = np.exp(-self.r * dt) * (
                            pu * self.call_option_values[self.steps - i + j - 1, i + 1] +
                            pm * self.call_option_values[self.steps - i + j, i + 1] +
                            pd * self.call_option_values[self.steps - i + j + 1, i + 1])

                self.put_option_values[self.steps - i + j, i] = np.exp(-self.r * dt) * (
                            pu * self.put_option_values[self.steps - i + j - 1, i + 1] +
                            pm * self.put_option_values[self.steps - i + j, i + 1] +
                            pd * self.put_option_values[self.steps - i + j + 1, i + 1])
